...
class PrivateRecipeAPITests(TestCase):
    """Test Authenticatd API requests"""
    def setUp(self):
        self.client = APIClient()
    ...
    def test_recipe_list_limited_to_user(self):
        """Test retriving a list of recipes to authenticated user"""
    ...
    def test_update_user_returns_error(self):
        ...
        new_user =  get_user_model().objects.create_user('user2@example.com','test123')
        recipe = create_recipe(user=self.user)
    ...
    def test_create_recipe_with_existing_ingredients(self):
        ...
        ingredient_old = Ingredient.objects.create(user=self.user, name='salt')
        payload = {
            'title': 'Pongal',
            'time_minutes': 60,
            'price': Decimal('4.50'),
    ...
    def test_update_recipe_assign_ingredient(self):
        ...
        ingredient_breakfast = Ingredient.objects.create(user=self.user, name='milk')
        recipe = create_recipe(user=self.user)
        recipe.ingredients.add(ingredient_breakfast)

        ingredient_lunch = Ingredient.objects.create(user=self.user, name='water')
